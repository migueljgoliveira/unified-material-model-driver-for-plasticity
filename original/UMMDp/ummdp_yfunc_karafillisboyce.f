...
c
c   local variables  : symbols in Karafillis-Boyce Yield Function's document
c
c   c_p       : c parameter
c   k_p       : k parameter
...
c   DDphiDDs(i,j,k)  : D2(Phi_i)/D(sigma_j)D(sigma_k)     (i=1~2, j=1~6, k=1~6)
c   DDjDDss(i,j,k)   : D2(J_i)/D(s_tilde_j)D(s_tilde_k)   (i=1~3, j=1~6, k=1~6)
c   DDphiDDj(i,j,k)  : D2(Phi_i)/D(J_j)D(J_k)             (i=1~2, j=1~3, k=1^3)
c   DDphiDDls(i,j,k) : D2(Phi_i)/D(S_tilde_j)D(S_tilde_k) (i=1~2, j=1~3, k=1^3)
c   DDlsDDj(i,j,k)   : D2(S_tilde_i)/D(J_j)D(J_k)         (i=1~3, j=1~3, k=1^3)
c   X12              : X12
c   X13              : X13
c   X23              : X23
...
c                                      1st order differential
      if ( nreq.ge.1 ) then
c --- check if there are two components of largeS are equal to each other
