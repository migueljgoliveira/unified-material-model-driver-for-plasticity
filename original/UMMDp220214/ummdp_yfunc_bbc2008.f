...
c
c   Title: Plane-Stress Yield Criterion For Highly-Anisotropic Sheet Metals
c   Authors: Dan-Sorin Comsa, Dorel Banabic
c            (Technical University of Cluj-Napoca, 15C. Daicoviciu, 400020 Cluj-Napoca, Romania)
c   Paper: Numisheet 2008, September 1-5, 2008 - Interlaken, Switzerland
c
c
c ---------------------------------------------------------------------------------------
...
c --- local variables
c     nds and ndk are used as s and k in the literature of BBC2008, respectively.
      integer :: nds, ndk
c
      nds=nint(pryld(2))
      ndk=nint(pryld(3))
...
c   <<local variables>>
c
c   se1: The factor to normalize equivalent stress calculated by original formula.
...
c          kCm(kp) = 2kp_C_2kp
c
c   Variables expressed by "xp" in this code are used as "x" in the BBC2008 paper.
c   
c   If sp and kp don't have appropriate values, we can't setup Lp, Mp, Np tensors and kCm.
c   
...
c   
c   Here, (x.y.10), (x.y.2d) etc are equation numbering used in a text / a paper
c   which will be released by JANCAE.
c   
c--------------------------------------------------------------------------------------------
...
c     Xp: coefficient tensor
c     s : stress vector
c     csp: CURRENT sp value, 1<=csp<=sp. Usually, it equals do-loop iterator "i".
c     lambda: see above.
c
c     local variables
c     nc: the number of components.
...
c In the bbc2008 section of the document "User subroutines for Metalic Plasticity model?",
c expression (x.y.2d) has "chi(gamma, beta)*s(beta)" (summation convention).
c In this routine, corresponding term is v(nc) obtained from jancae_mv().
