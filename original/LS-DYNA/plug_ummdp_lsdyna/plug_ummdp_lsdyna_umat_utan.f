...
c     dimension s1(6),s2(6),dpe(6),epsde(6)
c     dimension x1(100,6),x2(100,6),pe(6) !ht150908
c     dimension ddsdde(6,6)
      real, allocatable :: s1(:),s2(:),dpe(:),epsde(:)
...
c
      ...
      ntensor = nttl
c
      allocate( s1(nttl),s2(nttl),dpe(nttl),epsde(nttl) )
...
c       nttl=nnrm+nshr
c
c       deallocate( s1,s2,dpe,epsde )
c       deallocate( x1,x2,pe )!ht150908
c       deallocate( ddsdde )
c       allocate( s1(nttl),s2(nttl),dpe(nttl),epsde(nttl) )
c       allocate( x1(100,nttl),x2(100,nttl),pe(nttl) )!ht150908
c       allocate( ddsdde(nttl,nttl) )
c     endif
c
      call jancae_plasticity  ( s1,s2,epsde,
     &                          epsp,dp,dpe,de33,
     &                          x1,x2,mxpbs,
     &                          ddsdde,
     &                          nnrm,nshr,nttl,
...
c     changing LS-DYNA MATRIX(6,6)
      ...
      do i=1,3
        do j=1,3
          ...
          IF (i2.eq.3) i2=4
          if (j2.eq.3) j2=4
c         if (ddsdde(i,i) .lt. 1.0E-05) ddsdde(i,i)=1.0E-05
